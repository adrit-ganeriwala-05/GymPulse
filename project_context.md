...
There is no multi-user support, no cloud sync, and no export. Past workouts **can** be edited and deleted (long-press in History). An in-progress workout is persisted as a **draft** on every mutation and resumed on the next visit to `/active`. Tapping an exercise name in an expanded History card opens its **progress** (top set per training day, personal record).
...
│   │   ├── exercise_name.dart              # normalizeExerciseName: trim + case-fold, the one key rule (v4 name_key)
│   │   ├── read_models/
│   │   │   └── exercise_progress.dart      # ExerciseProgress read model: civil-day fold + PR rule (not an entity)
│   │   ├── entities/
│   │   │   ├── exercise.dart              # Exercise, ExerciseSet (weight is ALWAYS kg)
│   │   │   └── workout.dart                # Workout
│   │   ├── repositories/                   # abstract interfaces only
│   │   │   ├── progress_repository.dart    # read-only: getExerciseProgress(name)
│   │   │   ├── settings_repository.dart
│   │   │   ├── streak_repository.dart
│   │   │   └── workout_repository.dart     # + discardAllDrafts (A2-08)
...
│   │       ├── save_draft.dart / get_draft.dart / discard_draft.dart / discard_all_drafts.dart / record_draft_elapsed.dart
│   │       ├── delete_workout.dart
│   │       ├── get_exercise_progress.dart
│   │       └── update_streak.dart
│   ├── data/
│   │   ├── datasources/
│   │   │   ├── workout_database.dart       # single-flight open, onConfigure FK pragma, v4 schema + per-hop onUpgrade ladder
│   │   │   ├── workout_local_datasource.dart   # upsertWorkout (DELETE+INSERT, writes name_key), getWorkouts (done), getDraft, deleteWorkout, deleteDrafts
│   │   │   ├── progress_local_datasource.dart  # SQL aggregate: top set per finished workout for a name_key
...
│   │       ├── progress_repository_impl.dart   # normalises the name, folds rows via the domain read model
...
│       │   ├── home_screen.dart
│       │   ├── active_screen.dart
│       │   ├── history_screen.dart         # long-press edit/delete; exercise name tap → /exercise
│       │   ├── exercise_progress_screen.dart   # Feature D detail (FutureBuilder; no live badge yet)
...
│   ├── data/datasources/…                   # streak (29, injected clock) · workout on real SQLite via ffi (16) · migration hops (5) · progress aggregate (11)
...
### 4.6 Exercise progress (`exercise_progress_screen.dart`, Feature D)
- Entry: History → expand a card → tap an exercise name (chevron) → `push('/exercise', extra: name)`.
- Shows the personal record (weight × reps, day, number of training days) and, newest first, the top set per civil day as a bar proportional to the PR. `PR` badge on the earliest day that achieved the record; `= PR` on later equal days.
- Definitions: top set = max weight, then max reps; PR = lexicographic (weight, reps), earliest day keeps a tie. Two sessions on one day keep the stronger set. Drafts never count.
- Read path: `GetExerciseProgress` → `ProgressRepositoryImpl` (normalises the name) → `ProgressLocalDatasourceImpl` (one SQL aggregate, one row per finished workout) → `ExerciseProgress.fromWorkoutTopSets` (domain: civil-day fold + PR). `FutureBuilder`, fetched on mount; **no live "new PR" badge** — deferred until a reactive read path exists.

### 4.7 Settings (weight unit only)
...
Database file: `gympulse.db`, **version 4** (v2 `status`, v3 `timer_paused`, v4 `CHECK` on status + `one_draft` partial unique index + `exercises.name_key` — a table rebuild). `PRAGMA foreign_keys = ON` is set in `onConfigure` (the only hook where it takes effect — it is a no-op inside the create/upgrade transaction). `WorkoutDatabase.migrate` runs step N iff `old < N <= new`; `onDowngrade` deletes. The v4 rebuild renames the old tables away, creates the new ones with `_createDB`'s own DDL, copies, drops old children-first (a `DROP TABLE` on a parent with FKs ON cascades), backfills `name_key` from Dart.
...
CREATE UNIQUE INDEX one_draft ON workouts(status) WHERE status = 'draft'; -- v4: one draft at a time
...
CREATE TABLE exercises (
  ...
  name_key TEXT NOT NULL,             -- normalizeExerciseName(name): trim + Unicode case-fold (v4)
  FOREIGN KEY (workout_id) REFERENCES workouts(id) ON DELETE CASCADE
);
CREATE INDEX exercises_name_key ON exercises(name_key);
...
```
`position` columns preserve insertion order on read. Cascades fire on `deleteWorkout`, `deleteDrafts`, and on every `upsertWorkout` (an explicit `DELETE` by id then a plain `INSERT` — not `OR REPLACE`, which would resolve a `one_draft` conflict by deleting the other draft). A draft is an ordinary row with `status='draft'`; `getWorkouts` and the progress queries filter `status='done'` (the `CHECK` stops bogus values, it does not filter).
...
3. `StreakLocalDatasource` → singleton (constructed eagerly, needs prefs); `ProgressLocalDatasource` → lazy singleton.
4. Four repositories → singletons, each taking its datasource.
5. Fifteen use cases → singletons, each taking its repository.
...
- Routes `/`, `/active`, `/history` construct their BLoCs inline in the route `builder`, wired as `MultiBlocProvider`s scoped to that screen only. `/exercise` (pushed from History with `extra` = the exercise name) has no BLoC; the screen owns a `FutureBuilder`. **`/active` is a child route of `/`** (`GoRoute(path: 'active')` nested), so it is always poppable and `PopScope` runs on Android system back; Home is not recreated on the way to it and refreshes via `routeObserver` / `RouteAware.didPopNext`.
...
- Everything in §4, plus draft persistence/resume, edit and delete, exercise progress. 141 unit/widget tests (see `HANDOFF.md` §1 for the breakdown) + 1 on-device integration test per platform (seeds v1, walks the v1→v4 ladder on the real plugin); `flutter analyze` clean.
...
- `getWorkouts` is N+1 by design; revisit past ~2000 workouts. The progress read is the one aggregate SQLite computes.
- No `CHECK` constraints on `sets` (reps > 0); the v4 rebuild shape (`WorkoutDatabase._upgradeToV4`) is the template for adding them in v5.
- Feature D has no live "new PR" badge on save — needs a reactive read path (`HistoryBloc`) first.
